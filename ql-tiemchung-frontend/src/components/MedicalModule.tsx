...
import { Search, Edit, Trash2, Save, MapPin, Calendar, Syringe, Pill, X, ArrowLeft, User, Activity } from "lucide-react"; // Bổ sung User, Activity
...
export default function MedicalModule({ patients, setPatients, vaccines, triggerToast }: MedicalModuleProps) {
  // Thêm activeTab để quản lý việc chuyển tab
  const [activeTab, setActiveTab] = useState<"account_info" | "medical_records">("medical_records");
      ...
      <div className="border-b border-slate-200 flex space-x-2">
        ...
        <button
          ...
          className={`px-4 py-2.5 font-medium text-sm border-b-2 transition-colors flex items-center gap-2 ${
            activeTab === "medical_records" ? "border-blue-600 text-blue-600" : "border-transparent text-slate-500 hover:text-slate-800"
          }`}
        >
          <Activity className="w-4 h-4" /> Hồ sơ Bệnh án
        </button>
      </div>
