...
export default function AdminModule({ triggerToast = alert }: AdminModuleProps) {
  ...
  const [showAddSchedule, setShowAddSchedule] = useState(false);
    ...
    dateInput: '', thoiGian: '', loaiVacXin: '', soLuong: 0, doTuoi: '', diaDiem: '', ghiChu: '', selectedDoctors: [] as string[]
  });
  ...
  const resetAccountForm = () => {
    ...
    setScheduleForm({ dateInput: '', thoiGian: '', loaiVacXin: '', soLuong: 0, doTuoi: '', diaDiem: '', ghiChu: '', selectedDoctors: [] });
    setScheduleErrors({});
  ...
  const handleSaveSchedule = async (e: React.FormEvent) => {
    ...
    if (!scheduleForm.loaiVacXin.trim()) newErrors.loaiVacXin = 'Vui lòng nhập loại vắc xin';
    ...
    const payload = {
        ...
        loaiVacXin: scheduleForm.loaiVacXin,
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {activeTab === 'accounts' && (
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <button onClick={() => { resetAccountForm(); setShowAddAccount(true); }} className="bg-blue-600 text-white text-xs font-semibold px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-1 cursor-pointer whitespace-nowrap">
          ...
          <div className="flex flex-wrap items-center gap-2 pb-2">
            ...
            {ROLE_TABS.map(tab => {
              ...
              return (
                ...
                >
                  <span>{tab.label}</span>
                  <span className={`px-1.5 py-0.5 rounded-full text-[10px] ${roleFilter === tab.id ? 'bg-slate-600 text-slate-100' : 'bg-slate-100 text-slate-500'}`}>
                    {count}
                  </span>
                </button>
              )
                  ...
                  {currentAccounts.length > 0 ? currentAccounts.map((a) => (
                    <tr key={a.maTaiKhoan} className="hover:bg-slate-50/50">
                      ...
                      <td className="px-3 py-3">
                        <span className={`inline-block px-1.5 py-0.5 rounded text-[9px] font-bold border leading-tight ${getRoleBadgeStyle(a.phanQuyen)}`}>
                    ...
                    <tr>
                      <td colSpan={5} className="px-3 py-8 text-center text-slate-400 font-medium">Không tìm thấy dữ liệu.</td>
                    </tr>
                  )}
                </tbody>
              </table>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-slate-200 bg-slate-50/50">
                <span className="text-[11px] font-semibold text-slate-500">
                  Đang hiển thị <span className="text-slate-800">{((currentPage - 1) * ITEMS_PER_PAGE) + 1} - {Math.min(currentPage * ITEMS_PER_PAGE, filteredAccounts.length)}</span> trong tổng số <span className="text-slate-800">{filteredAccounts.length}</span> người dùng
                </span>
      ...
      {activeTab === 'schedules' && (
        <div className="space-y-6">
          ...
          {showAddSchedule && (
            <form onSubmit={handleSaveSchedule} noValidate className="bg-slate-50 p-6 rounded-xl border border-blue-200 space-y-4 animate-fade-in shadow-sm ring-1 ring-blue-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="md:col-span-1 bg-white p-3 rounded-lg border border-slate-200 flex flex-col justify-between shadow-xs">
                  <label className="block text-xs font-bold text-slate-700 mb-2">📅 Ngày tiêm <span className="text-red-500">*</span></label>
                  <input type="date" value={scheduleForm.dateInput} onChange={(e) => { handleDateChange(e.target.value); setScheduleErrors({...scheduleErrors, dateInput: ''}) }} className={`w-full px-2.5 py-1.5 border rounded-md text-xs outline-none transition-colors ${scheduleErrors.dateInput ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500'}`} />
                ...
                <div className="md:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Thời gian tiêm chủng <span className="text-red-500">*</span></label>
                    <input type="text" maxLength={50} placeholder="VD: 07:30 - 11:30" value={scheduleForm.thoiGian} onChange={(e) => { setScheduleForm({...scheduleForm, thoiGian: e.target.value}); setScheduleErrors({...scheduleErrors, thoiGian: ''}) }} className={`w-full bg-white px-3 py-2 border rounded-lg text-xs outline-none transition-colors ${scheduleErrors.thoiGian ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500'}`} />
                    {scheduleErrors.thoiGian && <p className="text-[10px] text-red-500 font-bold mt-1">{scheduleErrors.thoiGian}</p>}
                  </div>

                  {/* Ô Loại vắc xin đã chuyển thành Select Combobox */}
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Loại vắc xin đợt này <span className="text-red-500">*</span></label>
                    <select value={scheduleForm.loaiVacXin} onChange={(e) => { setScheduleForm({...scheduleForm, loaiVacXin: e.target.value}); setScheduleErrors({...scheduleErrors, loaiVacXin: ''}) }} className={`w-full bg-white px-3 py-2 border rounded-lg text-xs outline-none transition-colors cursor-pointer ${scheduleErrors.loaiVacXin ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500'}`}>
                      <option value="" disabled>-- Chọn vắc xin từ Kho --</option>
                      {/* Hãy đảm bảo bạn đã import và có state vaccineTypes như bài trước */}
                      {vaccineTypes && vaccineTypes.map(v => (
                        <option key={v.maLoaiVacXin} value={v.tenLoaiVacXin}>{v.tenLoaiVacXin}</option>
                      ))}
                    </select>
                    {scheduleErrors.loaiVacXin && <p className="text-[10px] text-red-500 font-bold mt-1">{scheduleErrors.loaiVacXin}</p>}
                  </div>

                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Số lượng vắc xin (liều) <span className="text-red-500">*</span></label>
                    <input type="number" min="1" value={scheduleForm.soLuong || ''} onChange={(e) => { setScheduleForm({...scheduleForm, soLuong: Number(e.target.value)}); setScheduleErrors({...scheduleErrors, soLuong: ''}) }} className={`w-full bg-white px-3 py-2 border rounded-lg text-xs outline-none transition-colors ${scheduleErrors.soLuong ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500'}`} />
                    {scheduleErrors.soLuong && <p className="text-[10px] text-red-500 font-bold mt-1">{scheduleErrors.soLuong}</p>}
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Độ tuổi khuyên dùng <span className="text-red-500">*</span></label>
                    <input type="text" maxLength={100} value={scheduleForm.doTuoi} onChange={(e) => { setScheduleForm({...scheduleForm, doTuoi: e.target.value}); setScheduleErrors({...scheduleErrors, doTuoi: ''}) }} className={`w-full bg-white px-3 py-2 border rounded-lg text-xs outline-none transition-colors ${scheduleErrors.doTuoi ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500'}`} />
                    {scheduleErrors.doTuoi && <p className="text-[10px] text-red-500 font-bold mt-1">{scheduleErrors.doTuoi}</p>}
                  </div>
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-semibold text-slate-600 mb-1">Địa điểm tổ chức <span className="text-red-500">*</span></label>
                    <input type="text" maxLength={255} value={scheduleForm.diaDiem} onChange={(e) => { setScheduleForm({...scheduleForm, diaDiem: e.target.value}); setScheduleErrors({...scheduleErrors, diaDiem: ''}) }} className={`w-full bg-white px-3 py-2 border rounded-lg text-xs outline-none transition-colors ${scheduleErrors.diaDiem ? 'border-red-500 focus:border-red-500 bg-red-50' : 'border-slate-200 focus:border-blue-500'}`} />
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="bg-white border border-slate-200 rounded-lg p-3 max-h-32 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {accounts.filter(a => a.phanQuyen?.toLowerCase().includes('y tế') || a.maQuyen === 5).map(bs => (
                      <label key={bs.maTaiKhoan} className="flex items-center gap-2 cursor-pointer hover:bg-slate-50 p-1 rounded transition-colors border border-transparent hover:border-slate-200">
                        <input
                          type="checkbox"
                        ...
                        />
                        <span className="text-xs text-slate-700 font-medium">{bs.hoTen} <span className="text-[9px] text-slate-400 font-mono">({bs.tenDangNhap})</span></span>
                      </label>
                    ))}
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea maxLength={1000} value={scheduleForm.ghiChu} onChange={(e) => setScheduleForm({...scheduleForm, ghiChu: e.target.value})} className="w-full bg-white px-3 py-2 border border-slate-200 rounded-lg text-xs h-16 resize-none focus:border-blue-500 outline-none transition-colors"></textarea>
                </div>
              </div>

              <div className="flex justify-end gap-2 pt-2 border-t border-slate-200">
                <button type="button" onClick={resetScheduleForm} className="px-4 py-2 border border-slate-200 rounded-lg text-xs font-semibold text-slate-600 bg-white hover:bg-slate-50 cursor-pointer">Thoát</button>
                <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg text-xs font-semibold shadow-sm hover:bg-blue-700 flex items-center gap-1.5 cursor-pointer"><Save className="w-4 h-4" /> Lưu thông tin</button>
              </div>
            </form>
          )}
                ...
                {filteredSchedules.length > 0 ? filteredSchedules.map((s) => (
                  <div key={s.maLichTiem} onClick={() => setSelectedSchedule(s)} className={`p-4 cursor-pointer transition-colors ${selectedSchedule?.maLichTiem === s.maLichTiem ? 'bg-blue-50/70 border-l-4 border-blue-600' : 'hover:bg-slate-50/50'}`}>
                    <div className="flex justify-between text-xs font-mono font-bold text-slate-400 mb-1">
                      ...
                      <span className="text-slate-500 font-sans">{s.ngay}/{s.thang}/{s.nam}</span>
                    </div>
              ...
              {selectedSchedule ? (
                <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-5 relative">
                  ...
                  <div className="absolute top-6 right-6 flex items-center gap-2">
                    <button onClick={() => handleEditSchedule(selectedSchedule)} className="text-blue-600 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-lg inline-flex items-center gap-1.5 text-xs font-bold transition-colors">
                      <Edit className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDeleteSchedule(selectedSchedule.maLichTiem)} className="text-red-600 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg inline-flex items-center gap-1.5 text-xs font-bold transition-colors">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 text-xs font-medium text-slate-700 flex flex-wrap gap-2">
                      {selectedSchedule.danhSachBacSi && selectedSchedule.danhSachBacSi.length > 0 ? selectedSchedule.danhSachBacSi.map((doc, idx) => (
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="bg-amber-50/50 border border-amber-200/60 p-3 rounded-lg text-xs text-slate-700 leading-relaxed whitespace-pre-line">{selectedSchedule.ghiChu || "Không có ghi chú."}</div>
                  </div>
                </div>
              ) : (
